...
rule extract_lambda_bam:
    ...
    output:
        bam_lambda=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda.bam",
    log:
        extract_lambda_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}_ExtractLambdaReads.log",
    conda:
...
rule extract_pUC19_bam:
    input:
        ...
        + "/results/4_alignment/bam/{sample}_w_ctrls_sorted.bam.bai",
    output:
        bam_pUC19=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19.bam",
    log:
        extract_pUC19_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}_ExtractpUC19Reads.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule filter_bam_lambda:
    input:
        ...
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda.bam",
    output:
        bam_lambda_filt=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt.bam",
    log:
        filt_bam_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}.filt_sambamba.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule filter_bam_pUC19:
    input:
        ...
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19.bam",
    output:
        bam_pUC19_filt=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt.bam",
    log:
        filt_bam_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}.filt_sambamba.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule move_umi_to_rx_tag_lambda:
    input:
        ...
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt.bam",
    output:
        bam_sorted_UMIrx=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt.rx.bam",
    log:
        bamtag_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}.rxbamtag.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule sort_bam_lambda:
    input:
        ...
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt.rx.bam",
    output:
        bam_lambda_filt_sort=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt_sort.rx.bam",
    log:
...
rule index_umi_bam_lambda:
    ...
    output:
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt_sort.rx.bam.bai",
    log:
...
rule UMI_dedup_lambda:
    input:
        bam_lambda_filt_sort=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt_sort.rx.bam",
        bam_lambda_filt_sort_bai=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt_sort.rx.bam.bai",
    output:
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_edit_distance.tsv",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_per_umi_per_position.tsv",
        config["resultsdir"] + "/results/9_methyl_ctrl/lambda/{sample}_per_umi.tsv",
        bam_dedup=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}_lambda_filt_dedup_umitools.bam",
    log:
        umitoolsdedup_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}.dedup_umitools.log",
    params:
...
rule move_umi_to_rx_tag_pUC19:
    ...
    output:
        bam_sorted_UMIrx=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt.rx.bam",
    log:
        bamtag_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}.rxbamtag.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule sort_bam_pUC19:
    input:
        ...
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt.rx.bam",
    output:
        bam_pUC19_filt_sort=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt_sort.rx.bam",
    log:
...
rule index_umi_bam_pUC19:
    ...
    output:
        config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt_sort.rx.bam.bai",
    log:
...
rule UMI_dedup_pUC19:
    input:
        bam_pUC19_filt_sort=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt_sort.rx.bam",
        bam_pUC19_filt_sort_bai=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt_sort.rx.bam.bai",
    output:
        config["resultsdir"] + "/results/9_methyl_ctrl/pUC19/{sample}_edit_distance.tsv",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_per_umi_per_position.tsv",
        config["resultsdir"] + "/results/9_methyl_ctrl/pUC19/{sample}_per_umi.tsv",
        bam_dedup=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}_pUC19_filt_dedup_umitools.bam",
    log:
        umitoolsdedup_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}.dedup_umitools.log",
    params:
        grouping_method=config["umi_tools_grouping_method"],
...
rule qc_methylation_lambda:
    ...
    output:
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}/{sample}_CpHRetentionByReadPos.txt",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}/{sample}_totalReadConversionRate.txt",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}/{sample}_CpGRetentionByReadPos.txt",
        bsstrand=config["resultsdir"]
        + "/results/9_methyl_ctrl/lambda/{sample}/{sample}_strand_qc.txt",
    params:
    ...
    log:
        qc_sh=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}_lambda.biscuit_qc.log",
        biscuit_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/lambda/{sample}_lambda.biscuit_bsstrand.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
...
rule qc_methylation_puc19:
    ...
    output:
        config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}/{sample}_CpHRetentionByReadPos.txt",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}/{sample}_totalReadConversionRate.txt",
        config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}/{sample}_CpGRetentionByReadPos.txt",
        bsstrand=config["resultsdir"]
        + "/results/9_methyl_ctrl/pUC19/{sample}/{sample}_strand_qc.txt",
    params:
    ...
    log:
        qc_sh=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}_pUC19.biscuit_qc.log",
        biscuit_log=config["resultsdir"]
        + "/logs/9_methyl_ctrl/pUC19/{sample}_pUC19.biscuit_bsstrand.log",
    conda:
        "../envs/twist_target.yaml"
    shell:
        """
