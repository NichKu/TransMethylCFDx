rule multiqc:
    input:
        expand(
            config["resultsdir"] + "/results/1_fastqc_init/{sample}_R1_fastqc.zip",
            sample=SAMPLES,
        ),
        expand(
            config["resultsdir"]
            + "/results/3_fastqc_trimmed/{sample}_R1_val_1_fastqc.zip",
            sample=SAMPLES,
        ),
        expand(
            config["resultsdir"]
        ...
        expand(
            config["resultsdir"] + "/results/5_dedup_consensus/{sample}{ending}",
            ending=["_edit_distance.tsv", "_per_umi_per_position.tsv", "_per_umi.tsv"],
            sample=SAMPLES,
        ),
        expand(
            config["resultsdir"]
    ...
    output:
        multiqc_report=config["resultsdir"]
    ...
    log:
        multiqc_log_res=config["resultsdir"]
        + "/logs/10_multiqc/multiqc_report.results.log",
        multiqc_log_cerebis=config["resultsdir"]
        + "/logs/10_multiqc/multiqc_report.cerebis.log",
    ...
    params:
        ...
        scan_directory_cerebis_res=config["resultsdir"]
        + "/results/9_methyl_ctrl/CEREBIS",
